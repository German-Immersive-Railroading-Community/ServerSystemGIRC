...
public class WorldsMenu extends PlayerInventory {
	...
	public WorldsMenu(Player player) {
		...
		setItem(calculateSize(Bukkit.getWorlds().size() + 9) - 5, createItem("Back", Material.ARROW), (itemstack) -> {new AdminMenu(player).open();});
		for(int i = 0; i < Bukkit.getWorlds().size() && i < 45; i++) {
			setItem(i, createItem(Bukkit.getWorlds().get(i).getName(), Material.LAPIS_BLOCK), (itemstack) -> {setWorld(itemstack, player);});
		}
	}
